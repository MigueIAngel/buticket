...
function EventCard({ eventId }: { eventId: Id<"events"> }) {
  ...
  const renderQueuePosition = () => {
    ...
    return (
      <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-100">
        <div className="flex items-center">
          <LoaderCircle className="w-4 h-4 text-blue-500 mr-2 animate-spin" />
  ...
  const renderTicketStatus = () => {
    ...
    if (userTicket) {
      return (
        <div className="mt-4 flex items-center justify-between p-3 bg-green-100 rounded-lg border border-green-200">
          <div className="flex items-center">
      ...
      <div className={`p-6 ${imageUrl ? "relative" : ""}`}>
        ...
        <div className="flex flex-col items-end gap-2 ml-4">
          ...
          {availability.purchasedTickets >= availability.totalTickets && (
            <span className="px-4 py-1.5 bg-red-700 font-semibold rounded-full text-sm">
              Sold Out
            </span>
          )}
        </div>
